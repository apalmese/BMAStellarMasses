...
def calc (inputDataDict, outfile, indir="simha/", lib="miles") :
	...
	# Comment -distanceModulus out for fsps versions <2.5, as their mags don't include distance modulus
        ...
        out_GR_P_MEMBER.append( GR_P_MEMBER[galaxy])
        out_RI_P_MEMBER.append( RI_P_MEMBER[galaxy])
        out_IZ_P_MEMBER.append( IZ_P_MEMBER[galaxy])
    ...
    out_GR_P_MEMBER = np.array(out_GR_P_MEMBER)
    out_RI_P_MEMBER = np.array(out_RI_P_MEMBER)
    out_IZ_P_MEMBER = np.array(out_IZ_P_MEMBER)
    ...
    col31=pf.Column(name='GR_P_MEMBER',format='E',array=out_GR_P_MEMBER)
    col32=pf.Column(name='RI_P_MEMBER',format='E',array=out_RI_P_MEMBER)
    col33=pf.Column(name='IZ_P_MEMBER',format='E',array=out_IZ_P_MEMBER)
    ...
    data = np.array([out_id, out_haloid, out_gr, out_stdgr, out_gi, out_stdgi, \
        ...
        out_zed,out_sfr, out_stdsfr, out_age, out_stdage, out_bestsp,out_bestzmet,out_zmet,out_GR_P_COLOR,out_RI_P_COLOR,out_IZ_P_COLOR,out_GR_P_MEMBER,out_RI_P_MEMBER,out_IZ_P_MEMBER,out_DIST_TO_CENTER, \
